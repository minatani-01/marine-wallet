...
export const API_BUDGET: Record<ApiName, { daily: number; monthly: number }> = {
  ...
  //
  // 1日の上限を 50 から一時的に上げてある。取り込みで「全部まとめて飲食」と
  // 入れたぶんの種別・ジャンルを、あとからまとめて取り込むため（0051）。
  // 46件のうち26件が未取得で、普段の検索と合わせると50回に収まらない。
  // 取り込み終わったら 50 に戻す。
  //
  // 月の上限は触らない。使い過ぎを止めているのは本当はこちらで、
  // 1日のぶんは「1日で月ぶんを使い切らない」ための均しに過ぎない。
  places_search: { daily: 100, monthly: 1000 },
}
